#!/usr/bin/env python3
"""
Read-only managed-artifact validator for cp-wiki — revision 3.
...
- distributed ``former_ids`` alias support plus the explicitly approved
  migration mappings as a transitional bootstrap;
- structured ``target_artifact`` descriptor support;
- type-specific status handling, including ``withdrawn`` for process drafts;
- embedded self-tests for the required inventory groups and guardrails;
...
Canonical repository location:
  ...
  validate_cpwiki_managed_artifacts_draft_v3.py

Usage:
  .venv/bin/python scripts/cp_wiki/validation/validate_cpwiki_managed_artifacts_draft_v3.py
  .venv/bin/python scripts/cp_wiki/validation/validate_cpwiki_managed_artifacts_draft_v3.py --self-test
  .venv/bin/python scripts/cp_wiki/validation/validate_cpwiki_managed_artifacts_draft_v3.py --strict-exit
"""
...
def validate_status(doc: Document, findings: list[Finding], *, full: bool) -> None:
    ...
    if status == "withdrawn" and "supersedes" in doc.frontmatter:
        add(
            findings,
            doc,
...
def validate_vault(vault: Path) -> tuple[list[Document], list[Finding], AliasIndex]:
    ...
    return documents, findings, aliases
...
def render_markdown(
    ...
    lines = [
        "# Managed Artifact Validation Report — Revision 3",
        "",
...
def write_reports(
    ...
    (report_dir / "validation-report-v3.json").write_text(
        json.dumps(payload, indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    (report_dir / "validation-report-v3.md").write_text(
        render_markdown(vault, documents, findings, aliases, generated.isoformat()),
        encoding="utf-8",
    )
    return report_dir
...
def run_self_tests() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="cpwiki-validator-v3-") as temp:
        ...
        documents, findings, aliases = validate_vault(vault)
        ...
        _, negative_findings, _ = validate_vault(negative)
...
def main() -> int:
    ...
    if args.self_test:
        try:
            ...
            return 1
        print("Managed-artifact validator v3 self-test passed.")
    ...
    severity_counts = Counter(finding.severity for finding in findings)
    profile_counts = Counter(doc.validation_profile for doc in documents)
    print("Managed-artifact validation v3 completed.")
